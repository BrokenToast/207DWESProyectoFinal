...
class Rest{
    public static function pedirTemperaturaEstacion(string $ubicacion){
        ...
        $aaa = file_get_contents($datosEstaciones['datos']);
        
        $curl = curl_init();

        curl_setopt_array($curl, array(
        CURLOPT_URL => "$datosEstaciones[datos]",
        ...
        echo $response;
        try{
            var_dump(json_decode($aaa,depth:2000000));
        }catch(Error $th){
            echo $th->getMessage();
        } catch (Exception $th) {
            echo $th->getMessage();
        ...
        if (!empty($codEstacion)) {
            ...
            return "No hay una estancion en esa zona";
        }
    }
}
